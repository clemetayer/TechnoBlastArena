...
func test_ready():
	...
	add_child(bullet)
	# then
	assert_eq(bullet.speed, 1)
	assert_eq(bullet.damage, 2)
	assert_eq(bullet.knockback, 3)
	...
	assert_eq(bullet.trail.modulate, Color.AQUA)
	assert_eq(bullet.sprite.modulate, Color.AQUA)
...
func test_parried():
	...
	assert_eq(bullet.trail.modulate, RuntimeUtils.PLAYER_INDICATOR_COLORS[1])
	assert_eq(bullet.sprite.modulate, RuntimeUtils.PLAYER_INDICATOR_COLORS[1])
