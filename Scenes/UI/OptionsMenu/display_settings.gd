...
func _process(_delta):
	warning_label.visible = IS_FROM_PAUSE_MENU


##### SIGNAL MANAGEMENT #####
func _on_display_type_button_item_selected(index: int) -> void:
	DisplayServer.window_set_mode(
		DisplayServer.WINDOW_MODE_WINDOWED if index == 0 else DisplayServer.WINDOW_MODE_FULLSCREEN
	)
